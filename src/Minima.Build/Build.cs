...
internal partial class Build : NukeBuild
{
    ...
    [Parameter] public static string Source { get; set; } = "https://nuget.alaplihaber.com/v3/index.json";
    ...
    public static string ModulesCachePath
    {
        get
        {
            if (string.IsNullOrWhiteSpace(_modulesCachePath))
            {
                ...
                if (string.IsNullOrWhiteSpace(_modulesCachePath))
                {
                    _modulesCachePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".mn-build", "cache");
                }
            }
        ...
        string artifactPath, bool prerelease)
    {
        ...
        var githubClient = new GitHubClient(new ProductHeaderValue("mn-build")) { Credentials = credentials };
