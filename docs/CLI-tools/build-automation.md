The `mn-build` contains a set of targets that are designed to automate the various build scenarios for solutions based on Minima and moreover, thanks to builder-server agnostic nature and cross-platform support, it can be integrated into any existing CI/CD pipeline with different build services, or used for local development scenarios.

![mn-build CLI](../media/cli-tools-2.png)


## Compile
```console
mn-build compile -configuration <Debug|Release>
```
...
+-------------------------------------------+--------+--------+--------+
| Minima.CatalogCsvImportModule.Core | 77,17% | 73,38% | 72,83% |
+-------------------------------------------+--------+--------+--------+
| Minima.CatalogCsvImportModule.Data | 56,47% | 52,15% | 56,86% |
+-------------------------------------------+--------+--------+--------+
...
```console
mn-build pack -configuration <Debug|Release>
```
...
```console
mn-build publishPackages -source <SOURCE> -apiKey <API_KEY>
```
...
### Example:
```console
mn-build publishPackages -source C:\local-nuget
```
...
```console
mn-build compress -configuration <Debug|Release>
```
...
```console
mn-build CloudUp -EnvironmentName <environment-name> -DockerUsername <docker-username> -DockerPassword <docker-password>
mn-build CloudDeploy -EnvironmentName <environment-name> -DockerUsername <docker-username> -DockerPassword <docker-password>
mn-build PrepareDockerContext -DockerUsername <docker-username> -EnvironmentName <environment-name>
```

The mn-build tool provides comprehensive Docker containerization capabilities for Minima applications. The container creation functionality automates the entire process of building, configuring, and deploying Docker containers for Minima platforms and modules.
...
1. **Prepares Docker Build Context** (`PrepareDockerContext` target):
   ...
   - Downloads the official Minima Dockerfile and helper scripts
   - Compiles and publishes the Minima platform to the Docker context
...
2. **Builds Docker Image** (`BuildImage` target):
   - Uses Docker to build a containerized image of the Minima application
...
mn-build BuildImage -DockerImageName mycompany/mystore:latest

# 3. Push to registry
mn-build PushImage -DockerImageName mycompany/mystore:latest
```
...
mn-build PrepareDockerContext -DockerUsername mycompany -EnvironmentName mystore
mn-build BuildImage -DockerImageName myregistry.com/mycompany/mystore:v1.0.0
```
